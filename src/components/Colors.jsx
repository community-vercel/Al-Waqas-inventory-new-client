// components/Colors.jsx - FINAL PERFECT VERSION
import React, { useState, useEffect } from 'react';
import { Plus, Trash2, Upload, Download, FileText, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
const Colors = () => {
  ...
  const [newColor, setNewColor] = useState({ name: '', codeName: '', hexCode: '#000000' });
  ...
  const handleHexInputChange = (value) => {
    ...
    setHexInput(cleaned);
    if (validateHexCode(cleaned) || cleaned === '#') {
 ...
 const handleCsvUpload = async (e) => {
  ...
  try {
    ...
    console.log('Uploading CSV file:', file.name, file.size, file.type);
  ...
  const paginatedColors = colors.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4">{editingColor ? 'Edit Color' : 'Add New Color'}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 text-center font-mono border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              maxLength={6}
            />
          </div>
              ...
              <div className="flex items-center gap-3">
                <div className=' flex-1'>
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 font-mono text-center border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    maxLength={6}
                  />
                </div>
                <input
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg flex items-center gap-6">
          <div className="w-20 h-20 rounded-lg shadow-lg border-4 border-white" style={{ backgroundColor: editingColor ? editingColor.hexCode : newColor.hexCode }}></div>
          <div>
            <p className="text-sm font-medium text-gray-600">Preview</p>
            <p className="text-lg font-mono">{(editingColor ? editingColor.hexCode : newColor.hexCode).toUpperCase()}</p>
            <p className="text-sm text-gray-500">
              {editingColor ? `${editingColor.name} (${editingColor.codeName})` : newColor.name ? `${newColor.name} (${newColor.codeName})` : 'Enter details'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex gap-3">
          {editingColor ? (
            <>
              <button onClick={updateColor} disabled={submitting} className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-medium">
                {submitting ? 'Updating...' : 'Update Color'}
              </button>
              <button onClick={() => setEditingColor(null)} className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded-lg font-medium">
                Cancel
              </button>
            </>
          ) : (
            <button onClick={addColor} disabled={submitting} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium">
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="px-6 py-4 bg-gray-50 border-b">
          <h3 className="text-lg font-semibold text-gray-900">All Colors ({colors.length})</h3>
        </div>
        ...
        {colors.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-400">Add your first color using the form above</p>
          </div>
        ) : (
          <>
            <div className="p-6">
